module Certification
  module Ship
    class ReviewsController < Certification::Ship::ApplicationController
      ...
      def update
        ...
        if @ship_review.update(ship_review_params)
          redirect_to next_certification_ship_reviews_path, notice: "Verdict recorded."
        else
      ...
      def next
        ...
        if candidate.nil?
          redirect_to certification_ship_reviews_path, notice: "Queue is empty." and return
        end
        claimed = ShipReview.atomic_claim!(candidate.id, current_user)
        if claimed
          redirect_to certification_ship_review_path(claimed)
        else
          new_skip = (skip_ids + [ candidate.id ]).uniq
          redirect_to next_certification_ship_reviews_path(skip: new_skip.join(","))
        end
      end
      ...
      def claim
        ...
        if claimed
          ...
          redirect_to certification_ship_reviews_path, alert: "Couldn't claim that review — someone else got it."
        end
      end
