...
class Project::Report < ApplicationRecord
    ...
    # These four never save a Project::Report row — Projects::ReportsController
    # branches on SLACK_ONLY_REASONS into a Slack-only ping instead of the
    # normal save path. should_not_have_been_approved is also the only one
    # excluded from REASONS below, since it's the only one with no history of
    # ever being persisted; the other three stay in REASONS/USER_REASONS so
    # admin views can still filter/count existing historical rows.
    ...
    # member), :not_approved (should_not_have_been_approved only, no approved
    # ship on record), or :throttled (already flagged recently).
    def self.flag_via_slack!(project:, reporter:, details:, reason:)
      ...
      # should_not_have_been_approved is tied to the approval, not just
      # (project, reporter), so a re-approval within the window doesn't
      # swallow a legitimate new flag. The other three reasons aren't tied to
      # any one approval cycle, so they're scoped to the project instead.
      # Checked as a plain exists? first — an outage making `write` return
      # falsy shouldn't be indistinguishable from "already flagged" and
      # silently drop a report.
